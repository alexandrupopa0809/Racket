...
;(define-struct state_struct (bird_struct pipes_list variables_struct) #:transparent)
(define-struct bird_struct (x y v-y ) #:transparent)
(define-struct variables_struct (gravity momentum scroll-speed) #:transparent)
(define-struct pipe_struct (gap-y x) #:transparent)
(define-struct score_struct (s) #:transparent)
(define-struct state_struct (bird_struct pipes_list variables_struct score_struct) #:transparent)

(define score_0 (score_struct 0))
(define bird_0 (bird_struct bird-x bird-initial-y 0))
(define variables_0 (variables_struct initial-gravity initial-momentum initial-scroll-speed))
;TODO 8
...
(define pipe_0 (pipe_struct (+ added-number (random random-threshold)) scene-width));
...
(define state_0 (state_struct bird_0 (list pipe_0) variables_0 0))
...
(define (get-bird state)
  (state_struct-bird_struct state))
...
(define (next-state-bird bird gravity)
  (struct-copy bird_struct bird[y (+ (get-bird-y bird) (get-bird-v-y bird))]
...
(define (change current-state pressed-key)
  (cond [(key=? pressed-key " ")
         (struct-copy state_struct current-state
                      [bird_struct
                       (next-state-bird-onspace
                        (get-bird current-state) initial-momentum)])]
        [else current-state]))
...
(define (move-pipes pipes scroll-speed)
  (if (null? pipes)
      null
      (cons (struct-copy pipe_struct (car pipes)
                         [x (- (get-pipe-x (car pipes)) scroll-speed)])
            (move-pipes (cdr pipes) scroll-speed))))
...
(define (add-more-pipes pipes)
  (if (null? pipes)
      ...
      (if (< (length pipes) no-pipes)
          (append pipes
                  (list (struct-copy pipe_struct
                                     (last pipes)[x (+ (+ pipe-width pipe-gap)
...
(define (get-score state)
  (state_struct-score_struct state))
...
(define (invalid-state? state)
  (or (check-pipe-collisions (get-bird state) (get-pipes state))
...
(define (check-pipe-collisions bird pipes)
  (if (null? pipes)
      #f
      (or (check-collision-rectangles (make-posn (get-bird-x bird) (get-bird-y bird))
                                      (make-posn (+ (get-bird-x bird) bird-width)
                                                 (+ (get-bird-y bird) bird-height))
                                      (make-posn (get-pipe-x (car pipes)) 0)
                                      (make-posn (+ (get-pipe-x (car pipes)) pipe-width)
                                                 (get-pipe-gap-y (car pipes))))
          (check-collision-rectangles (make-posn (get-bird-x bird) (get-bird-y bird))
                                      (make-posn (+ (get-bird-x bird) bird-width)
                                                 (+ (get-bird-y bird) bird-height))
                                      (make-posn (get-pipe-x (car pipes))
                                                 (+ (get-pipe-gap-y (car pipes)) pipe-self-gap))
                                      (make-posn (+ (get-pipe-x (car pipes)) pipe-width)
                                                 ground-y))
...
(define (next-state state)
  (struct-copy state_struct state
               [bird_struct (next-state-bird (get-bird state) initial-gravity)]
               [pipes_list (next-state-pipes (get-pipes state) initial-scroll-speed)]
               [score_struct (+ 0.1 (get-score state))]))
...
(define (draw-frame state)
  (place-image
   ...
   (place-image
    ...
    (place-image
     (score-to-image (get-score state))
     text-x
     text-y
     (place-pipes (get-pipes state) initial-scene)
     ))))

(define lower-pipe-image (rectangle pipe-width (quotient pipe-height 2) "solid" "green"))
(define upper-pipe-image (rectangle pipe-width (quotient pipe-height 2) "solid" "green"))
(define white-rectangle (rectangle pipe-width pipe-self-gap "solid" "white"))
...
(define (place-pipes pipes scene)
  (if (null? pipes)
      ...
      (place-pipes (cdr pipes)
                   (place-image
                    (rectangle pipe-width (get-pipe-gap-y (car pipes)) "solid" "green")
                    (+ (get-pipe-x (car pipes)) (quotient pipe-width 2))
                    (quotient (get-pipe-gap-y (car pipes)) 2)
                    (place-image
                     (rectangle pipe-width (- pipe-height (get-pipe-gap-y (car pipes)) pipe-self-gap) "solid" "green")
                     (+ (get-pipe-x (car pipes)) (quotient pipe-width 2))
                     (quotient (+ scene-height (get-pipe-gap-y (car pipes)) pipe-self-gap) 2)
                     scene
                     )))))
